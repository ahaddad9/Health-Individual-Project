...
MODEL_FEATURES = ["child_mort", "water", "reach"]
...
with tab_dash:
    ...
    with col_b:
        ...
        line = px.line(pl, x="mid", y="mean_LE", markers=True, text="mean_LE",
    ...
    st.caption("Each pillar plotted against life expectancy across all countries. "
               "Fewer child deaths, more clean water, and broader coverage each go "
               "with longer lives.")
    ...
    pillar_chart(p1, "child_mort", "Prevention",
                 "Countries with fewer child deaths tend to live longer.")
    pillar_chart(p2, "water", "Environment",
                 "More access to safe water goes with longer lives.")
    pillar_chart(p3, "reach", "Reach",
                 "Broader health service coverage goes with longer lives.")
...
with tab_model:
    ...
    st.caption("Standardized regression of the three pillars on life expectancy for "
               "the selected year. Every predictor is on the same scale, so bar length "
               "shows how much improving each pillar moves life expectancy. This is "
               "association from one year of data, not proof of cause.")
    ...
    st.caption("If the model were perfect, every country would sit on the diagonal. "
               "The three pillars predict life expectancy closely. The US sits just "
               "below the line, about where its health fundamentals put it, even though "
               "it spends far more than any other country. The extra money did not buy "
               "a higher position.")
